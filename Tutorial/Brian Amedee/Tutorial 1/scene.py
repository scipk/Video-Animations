...
class NameOfAnimation(Scene):
    def construct(self):
        grid = NumberPlane(x_range=[-7,7,1], y_range=[-4,4,1]).add_coordinates()
        self.add(grid)

        box = Rectangle(stroke_color=GREEN_C, stroke_opacity=0.7, fill_color=RED_B, fill_opacity=0.5, height=1, width=1)
        self.add(box)

        dot = Dot(color=WHITE).move_to(box.get_center())
        self.add(dot)
        
        box_and_dot = VGroup(box, dot)

        self.play(box_and_dot.animate.shift(RIGHT*2), run_time=2)
        self.play(box_and_dot.animate.shift(UP*3), run_time=2)
        self.play(box_and_dot.animate.shift(DOWN*5+LEFT*5), run_time=2)
        self.play(box_and_dot.animate.shift(UP*1.5+RIGHT*1), run_time=2)
        self.play(box_and_dot.animate.shift(UP*1.0+RIGHT*2.5), run_time=2)
        self.wait(1)

        self.play(FadeOut(box_and_dot, grid))
...
class FittingObjects(Scene):
    def construct(self):
        ...
        axes = Axes(x_range=[-3,3,1], y_range=[-3,3,1], x_length=6, y_length=6).to_edge(LEFT, buff=0.5)
        circle = Circle(stroke_width=6, stroke_color=YELLOW, fill_color=RED_C, fill_opacity=0.8).set_width(2).to_edge(DR, buff=0)

...
class Derivatives(Scene):
    def construct(self):
        ...
        func1: ParametricFunction = plane1.plot(
            lambda x: (1/3) * x ** 3, x_range=[-3,3], color=RED_C
        )
        func1_lab = (
            MathTex(r"f(x)=\frac{1} {x}^{3}")
            .set(width=2.5)
        ...
        func2: ParametricFunction = always_redraw(
            lambda: plane2.plot(
        ...
        hline = always_redraw(
            lambda: plane2.get_horizontal_line(
                plane2.input_to_graph_point(k.get_value(), func2)
            )
        )
        ...
        self.add(moving_slope, hline, func2, slope_value, slope_value_text, dot)
...
class AdditiveFunctions(Scene):
    def construct(self):
        ...
        func1: ParametricFunction = axes.plot(lambda x: x ** 2, x_range=[0,2], color=YELLOW)
        func1_lab = (
            MathTex("y={x}^{2}").scale(0.8).next_to(func1, UR, buff=0).set_color(YELLOW)
        )

        func2: ParametricFunction = axes.plot(lambda x: x, x_range=[0,2], color=GREEN)
        func2_lab = (
            MathTex("y=x").scale(0.8).next_to(func2, UR, buff=0).set_color(GREEN)
        )

        func3: ParametricFunction = axes.plot(lambda x: x ** 2 + x, x_range=[0,2], color=PURPLE_D)
        func3_lab = (
            MathTex("y={x}^{2} + x")
            .scale(0.8)
        ...
        for k in np.arange(0.2, 2.1, 0.2):
            ...
            self.play(FadeOut(line1, line2), Create(line3))
        ...
        self.wait()
        for k in range(20):
            rect: Rectangle = area1[k]
            self.play(area2[k].animate.shift(UP * rect.get_height()))
        self.wait(2)

##
# I copied Brian's code from his GitHub for this last 
# animation, just to see what I understand from it.
##
